...
# MCP 이름을 봤다"에 [FILE]을 붙였다. 도구 목록·도구 스키마·시스템 메시지를 본 것은
# 파일 읽기도 웹 조회도 아니다.
#
# 이 검사가 따로 필요한 이유는 **미탐 구멍**이다. [FILE]은 세션에 Read·Glob·Bash가
# 하나라도 있으면 만족되는데, "도구 목록을 봤다"는 근거는 그 호출들과 무관하다. 즉 모델이
# 다른 이유로 파일을 한 번 읽었으면 이 오용은 그냥 통과한다 — 런 H·S4·M이 잡힌 것은
# 우연히 FILE 계열 호출이 없었기 때문이다. 그래서 **도구 호출 유무와 무관하게** 잡는다.
#
# 좁게 잡는다. "MCP"나 "목록"만으로는 걸리지 않는다 — "[FILE] MCP 설정 파일을 읽어
# 연결된 서버 목록을 확인했다"는 실제 파일 읽기이므로 통과해야 한다.
TOOL_INVENTORY = re.compile(
    r"도구\s*목록|툴\s*목록|도구\s*스키마|tool\s*list"
    r"|세션에[^.。]{0,12}(연결|붙어|로드|리스트|등재)"
    r"|이\s*환경에[^.。]{0,12}(연결|붙어|로드|리스트)"
    r"|시스템\s*메시지"
)
...
def find_violations(message, used):
    ...
    for sentence in sentences(message):
        ...
        if present:
            # 근거의 출처가 이 세션의 도구 인벤토리면 어떤 도구 호출로도 정당화되지 않는다.
            # 도구 호출 유무를 보지 않는 유일한 검사다 — 이유는 상단 TOOL_INVENTORY 주석.
            if TOOL_INVENTORY.search(sentence):
                for tag in present:
                    violations.append(("도구목록 오용", sentence))
                continue
            for tag in present:
...
def main():
    ...
    # 도구목록 오용은 3회 관찰된 재발 패턴이므로 전용 안내를 붙인다(런 H·S4=Opus, M=Sonnet).
    # 일반 반송문의 태그 정의는 문장 끝에 묻혀 있어 교정이 여러 회차 걸렸다.
    if any(kind == "도구목록 오용" for kind, _ in violations):
        reason += (
            "\n\n위 항목 중 일부는 **이 세션의 도구 목록·도구 스키마·시스템 "
            "메시지를 근거로 태그를 붙인 것**입니다. 그건 파일 읽기도 웹 조회도 아닙니다 — "
            "도구가 목록에 있다는 사실과 그 도구를 실행해 얻은 결과는 다릅니다. 이 오용은 "
            "상위 티어에서도 3회 관찰됐습니다.\n"
            "고치는 방법: 태그를 떼고 사실로만 서술하세요. 예 — "
            "\"[FILE] 세션에 붙어 있는 Notion MCP\" → \"이 세션에 Notion MCP가 "
            "연결돼 있다(실행하지는 않았다)\". 실제로 그 도구를 호출해 결과를 얻었다면 "
            "그 호출이 기록에 남으므로 태그를 유지해도 됩니다."
        )
